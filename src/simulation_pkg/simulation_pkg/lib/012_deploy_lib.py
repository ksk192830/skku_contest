...
def get_base_path(extra_dirs=None, repeat_last=False):
    ...
    # This file: <ws>/src/simulation_pkg/simulation_pkg/lib/012_deploy_lib.py
    # package root we want: <ws>/src/simulation_pkg
...
def load_model(entity_name, model_name, random_coordinates):
    ...
    # NOTE:
    # spawn_entity.py shebang is `/usr/bin/env python3`.
    # In this environment, `python3` can resolve to Homebrew Python(3.14),
    # which breaks ROS Humble(rclpy for 3.10).
    # Force system Python(3.10) to keep gazebo_ros spawn stable.
    model_file = get_model(model_name)
    spawn_script = '/opt/ros/humble/lib/gazebo_ros/spawn_entity.py'
    cmd = (
        f"/usr/bin/python3 {spawn_script} "
        f"-file {model_file} -entity {entity_name} "
